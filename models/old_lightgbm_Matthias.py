...
from sklearn.model_selection import KFold  # <--- This is the one you are missing!
# ============================================================
# Imports
# ============================================================
...
transactions_path = r'C:\Users\shop\Downloads\transactions_2016_2017.csv'
customer_path = r'C:\Users\shop\Downloads\customer_clv_train.csv'
test_path = r'C:\Users\shop\Downloads\customer_clv_test.csv'
...
df
# %%
# 1. Create Advanced Behavioral Features from the Merged Data
print("Aggregating transaction behavior...")

# Identify returns (where returned_to_shop_id is not empty)
df['is_returned'] = df['returned_to_shop_id'].notna().astype(int)

# Create the summary features
agg_features = df.groupby('cust_id').agg({
    'sale_revenue': ['sum', 'mean', 'std'],      # Basic spend stability
    'sale_discount_applied': 'mean',             # Bargain hunter signal
    'is_returned': 'mean',                       # Return rate (MAE gold)
    'prod_brand': 'nunique',                     # Brand loyalty
    'prod_id': 'count',                          # Transaction volume
    'order_date': [lambda x: (x.max() - x.min()).days, 'nunique'] # Tenure & Frequency
}).reset_index()

# Flatten the column names
agg_features.columns = [
    'cust_id', 'total_rev', 'avg_rev', 'rev_volatility', 
    'avg_discount', 'return_rate', 'brand_loyalty', 
    'total_items', 'customer_tenure', 'order_count'
]

# 2. CREATE THE FINAL MODELING TABLES
# This ensures we have exactly one row per customer for the model
customer_model_df = agg_features.copy()

# Note: You'll need to merge your TARGET variable (revenue_2018_2019) 
# back onto this agg_features table if it's in a separate file.
print(f"Final Aggregated shape: {customer_model_df.shape}")

# 3. ATTACH THE TARGET (The 'Answer' the model learns from)
# We assume 'df_customer' has the 'revenue_2018_2019' column
target_df = df_customer[['cust_id', 'revenue_2018_2019']].drop_duplicates()
customer_model_df = customer_model_df.merge(target_df, on='cust_id', how='left')

# 4. PREPARE THE TEST SET
# We need a separate version of these features for the 29,148 test customers
# We use the same features but only for the IDs in your test file
# 3. ATTACH THE TARGET
target_df = df_customer[['cust_id', 'revenue_2018_2019']].drop_duplicates()
customer_model_df = customer_model_df.merge(target_df, on='cust_id', how='left')

# 4. PREPARE THE TEST SET (Safe Version)
# We use 'df_customer' to find the IDs that have 0 revenue (those are usually the test ones)
# OR simply use the original df_customer list
try:
    customer_model_df_test = customer_features_test[['cust_id']].merge(agg_features, on='cust_id', how='left').fillna(0)
except NameError:
    # If customer_features_test is missing, we use df_customer but filter for the test IDs
    print("Variable 'customer_features_test' not found. Using df_customer fallback...")
    customer_model_df_test = df_customer[['cust_id']].drop_duplicates().merge(agg_features, on='cust_id', how='left').fillna(0)# %% [markdown]
# ## Data Types
...
def build_customer_features(df_input):
    ...
    # ... (all your existing code for recent_revenue_ratio, numeric_cols, etc.)

    # ============================================================
    # NEW PROXY FEATURES (Size & Season)
    # ============================================================
    
    # 1. Size Progression (The "Outgrowth" Signal)
...
# 1. CAPTURE IDs BEFORE DROPPING
# We save the IDs in a separate variable so they can't be "lost"
test_ids = customer_features_test['cust_id'].copy()

# 2. PREPARE TEST FEATURES
# We drop the columns but keep the dataframe aligned
customer_model_df_test = customer_features_test.drop(columns=cols_to_drop).copy()

# 3. SELECT COMMON FEATURES (Safety Check)
# Ensure Train and Test have the EXACT same columns in the EXACT same order
target = 'revenue_2018_2019'
exclude = ['cust_id', target, 'first_order_date', 'last_order_date']

feature_cols = [c for c in customer_model_df.columns 
               if c in customer_model_df_test.columns 
               and c not in exclude]

X = customer_model_df[feature_cols]
y = customer_model_df[target]
X_test_final = customer_model_df_test[feature_cols]

print(f"DEBUG: Model training on {len(feature_cols)} features")
print(f"DEBUG: Test set row count: {len(X_test_final)}")

# 4. TUNED PARAMETERS FOR < 62.3
params = {
...
oom_preds = np.zeros(len(X_test_final))
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
    ...
    oom_preds += model.predict(X_test_final) / kf.get_n_splits()

# ============================================================
# FINAL STEP: SAVE THE SUBMISSION (With Aggressive Threshold)
# ============================================================

# 1. Base Predictions
submission = pd.DataFrame({
    'cust_id': test_ids,
    'revenue': np.maximum(0, oom_preds)
})

# 2. The "Floor" Filter
# If the model predicts less than 5.0, it's likely noise. Set to 0.
submission.loc[submission['revenue'] < 5.0, 'revenue'] = 0

# 3. The "Recency" Filter (The 61.717 Winner)
if 'recency_days' in customer_model_df_test.columns:
    # Anyone over 400 days is very risky
    submission.loc[customer_model_df_test['recency_days'] > 400, 'revenue'] *= 0.2
    # Anyone over 500 days is almost certainly 0
    submission.loc[customer_model_df_test['recency_days'] > 500, 'revenue'] = 0

# 4. Save
submission.to_csv(r'C:\Users\shop\Downloads\submission_reset.csv', index=False)
