...
# ============================================================
# We force the model to respect business logic:
# 1  = As feature increases, predicted revenue must increase (or stay same)
# -1 = As feature increases, predicted revenue must decrease (or stay same)

monotone_dict = {}
for col in X_train.columns:
    if col in ['total_spent', 'log_total_spent', 'n_orders', 'revenue_last_90d', 'n_products']:
        monotone_dict[col] = 1   # More history = More future value
    elif col in ['recency_days', 'avg_days_between_orders']:
        monotone_dict[col] = -1  # Longer silence = Lower future value
    else:
        monotone_dict[col] = 0   # No constraint for colors, brands, etc.

# ============================================================
# 2. Initialize and Train CatBoost
# ============================================================
# We use MAE because it's the primary competition metric.
...
# ============================================================
# 3. Validation & Competition Metrics
# ============================================================
y_pred_val = model_cb.predict(X_val)
y_pred_val = np.maximum(y_pred_val, 0) # Revenue cannot be negative

mae_val = mean_absolute_error(y_val, y_pred_val)
spearman_val, _ = stats.spearmanr(y_val, y_pred_val)

print(f"\n--- Validation Results ---")
print(f"MAE: {mae_val:.4f}")
print(f"Spearman Correlation: {spearman_val:.4f}")

# Stage 1: Create binary target for the classification task
y_train_binary = (y_train > 0).astype(int)
y_val_binary = (y_val > 0).astype(int)

# Stage 2: Create a dataset of ONLY spenders for the regression task
train_spender_idx = y_train > 0
X_train_spenders = X_train[train_spender_idx]
y_train_spenders = y_train[train_spender_idx]
from catboost import CatBoostClassifier, CatBoostRegressor

# --- PART A: CLASSIFIER (Will they spend?) ---
...
model_classifier.fit(X_train, y_train_binary, eval_set=(X_val, y_val_binary))

# --- PART B: REGRESSOR (Log-Transformed) ---
# We train on the Log of the revenue to stabilize the MAE
y_train_log = np.log1p(y_train_spenders)

model_regressor_log = CatBoostRegressor(
    iterations=1000,
    learning_rate=0.05,
    loss_function='MAE', # Still MAE, but on the log-scale
    monotone_constraints=monotone_dict,
    random_seed=42,
    verbose=200
)

model_regressor_log.fit(X_train_spenders, y_train_log)

# --- FINAL PREDICTION (Back-transform) ---
# 1. Get probability from the Classifier
prob_spending = model_classifier.predict_proba(X_test)[:, 1]

# 2. Get log-prediction and convert back to Euros using expm1
pred_log_amount = model_regressor_log.predict(X_test)
pred_euro_amount = np.expm1(pred_log_amount)

# 3. Combine
final_predictions_log = prob_spending * pred_euro_amount

# 4. Evaluate
mae_log = mean_absolute_error(y_test, final_predictions_log)
spearman_log, _ = stats.spearmanr(y_test, final_predictions_log)

print(f"Log-Hurdle MAE: {mae_log:.4f}")
print(f"Log-Hurdle Spearman: {spearman_log:.4f}")
