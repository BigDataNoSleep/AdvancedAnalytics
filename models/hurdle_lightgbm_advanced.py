...
# Faster, more aggressive hyperparameters for iteration
clf_params = {
    'objective': 'binary',
    'metric': 'binary_logloss',
    'boosting_type': 'gbdt',
    'learning_rate': 0.02,       # Faster learning for the gatekeeper
    'num_leaves': 31,
...
# HURDLE SETTINGS: 'soft' (Prob * Amount) is proven better for this data
HURDLE_TYPE = 'soft' 
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
    ...
    clf_model = lgb.train(
        clf_params, dtrain_clf, num_boost_round=1000,
        valid_sets=[dval_clf],
    ...
    amount_val = reg_model.predict(X_f_val)
    amount_test = reg_model.predict(test_unlabelled)
    ...
    if HURDLE_TYPE == 'soft':
        ...
        # Find best threshold for this fold to minimize MAE
        best_t, best_mae = 0.5, float('inf')
        for t in np.linspace(0.3, 0.7, 9):
            fold_mae = mean_absolute_error(y_f_val, np.where(prob_val > t, amount_val, 0))
            if fold_mae < best_mae:
                best_t, best_mae = t, fold_mae
        
        print(f"  [Hurdle] Best threshold for Fold {fold+1}: {best_t:.2f} (MAE: {best_mae:.4f})")
        oof_preds[val_idx] = np.where(prob_val > best_t, amount_val, 0)
        test_preds += np.where(prob_test > best_t, amount_test, 0) / kf.get_n_splits()
